...
@Table(name="identitydocuments") //is used to give table name in database
public class IdentityDocuments implements Serializable {
    @Id // is used to make primary key to a column in table
    @Column(name="iid") //is used to give column name in database
    @GeneratedValue(strategy = GenerationType.IDENTITY) //is used to autogenerate primary values
    long iid;
